"""
AlphaZero — 9x9 Gomoku.

Base: alpha_zero2_chk (proven hyperparameters, smaller network, no curriculum).
Speed: batched self-play from az_gomuku4 (all games evaluated in a single GPU pass).
"""
...
CHECKPOINT_PATH = "models_az2/checkpoint.pt"
BUFFER_PATH     = "models_az2/buffer.pkl"
...
def train(n_iters: int = 150):
    os.makedirs("models_az2", exist_ok=True)

    net = AZNet().to(DEVICE)
    opt = torch.optim.SGD(net.parameters(), lr=LR, momentum=MOMENTUM,
                          weight_decay=WEIGHT_DECAY)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        opt, milestones=[50, 80, 105, 130], gamma=0.5)
    ...
    for it in range(start_iter, n_iters + 1):
        ...
        torch.save(net.state_dict(), f"models_az2/az_iter{it:04d}.pt")
