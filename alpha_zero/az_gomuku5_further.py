"""
AlphaZero — 9x9 Gomoku, v5-further (attempt 4).

Continues training from v5 iter 118 checkpoint (models_az5_118/).
Two-phase "spike-and-settle" strategy, informed by attempts 1-2.

Strategy:
  Phase 1 (iters 119-120): LR=0.008, seeded with v5 buffer — kick weights into better basin
  Phase 2 (iters 121-148): cosine 0.002 → 5e-4 — lock in and refine from new basin
"""
...
# ── Two-phase config ──
BUFFER_SIZE   = 100_000    # seeded from v5, then rolls over
BATCH_SIZE    = 256
LR_SPIKE      = 8e-3       # phase 1: high LR to escape basin (2 iters)
LR_SETTLE     = 2e-3       # phase 2: cosine from here down
LR_MIN        = 5e-4       # phase 2: cosine minimum
SPIKE_ITERS   = 2          # how many iters at high LR
WEIGHT_DECAY  = 1e-4
MOMENTUM      = 0.9
TRAIN_STEPS   = 400        # more gradient steps per iter (was 200)
GAMES_PER_ITER= 50         # same as v5
N_FURTHER     = 30         # total further-training iterations

# ── Paths ──
SOURCE_CHECKPOINT = "models_az5_118/checkpoint.pt"   # v5 iter 118 checkpoint
SOURCE_BUFFER     = "models_az5_118/buffer.pkl"      # v5 iter 118 buffer (seed)
SAVE_DIR          = "models_az5_further4"
...
# ── 7. Main loop — warm restart from v5 ──────────────────────────────────────
...
def train():
    ...
    # Phase 1 starts with spike LR; phase 2 scheduler created after spike
    opt = torch.optim.SGD(net.parameters(), lr=LR_SPIKE, momentum=MOMENTUM,
                          weight_decay=WEIGHT_DECAY)
    # Cosine scheduler for phase 2 — created/loaded later
    scheduler = None

    buf: deque = deque(maxlen=BUFFER_SIZE)

    start_iter = 119
    spike_end  = 118 + SPIKE_ITERS   # iter 120

    # Check for existing further-training checkpoint first
    if os.path.exists(CHECKPOINT_PATH):
        ...
        if ckpt.get("scheduler"):
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                opt, T_max=N_FURTHER - SPIKE_ITERS, eta_min=LR_MIN)
        ...
        # Load model weights from v5 checkpoint (fresh optimizer, seeded buffer)
        ...
        net.load_state_dict(ckpt["model"])
        print(f"Loaded v5 weights from iter {ckpt['iter']} (fresh optimizer)")

        # Seed buffer with v5 data so spike trains on high-quality examples
        if os.path.exists(SOURCE_BUFFER):
            with open(SOURCE_BUFFER, "rb") as f:
                src_buf = pickle.load(f)
            buf.extend(src_buf)
            print(f"Seeded buffer from v5 ({len(buf)} examples)")

    print(f"AlphaZero 9x9 Gomoku v5-further4 | device={DEVICE} | "
          f"params={sum(p.numel() for p in net.parameters()):,}")
    print(f"Phase 1: LR={LR_SPIKE} for {SPIKE_ITERS} iters | "
          f"Phase 2: cosine {LR_SETTLE} → {LR_MIN} for {N_FURTHER - SPIKE_ITERS} iters")

    end_iter = 118 + N_FURTHER

    for it in range(start_iter, end_iter + 1):
        # ── Phase transition: switch from spike LR to cosine settle ──
        if it == spike_end + 1 and scheduler is None:
            for pg in opt.param_groups:
                pg['lr'] = LR_SETTLE
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                opt, T_max=N_FURTHER - SPIKE_ITERS, eta_min=LR_MIN)
            print(f"── Phase 2: LR dropped to {LR_SETTLE}, cosine annealing starts ──")
        ...
        if scheduler is not None and it > spike_end:
            scheduler.step()

        current_lr = opt.param_groups[0]['lr']
        phase = "SPIKE" if it <= spike_end else "settle"
        print(f"Iter {it:3d} | buf={len(buf):6d} | loss={tot_loss/TRAIN_STEPS:.4f} "
              f"(pol={tot_pol/TRAIN_STEPS:.4f} val={tot_val/TRAIN_STEPS:.4f}) | lr={current_lr:.6f} [{phase}]")

        # Save with same naming convention so eval scripts work
