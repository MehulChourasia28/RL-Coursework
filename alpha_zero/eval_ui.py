...
HINT_COLOR    = (50,  205, 50)
HINT_RING_W   = max(2, CELL_SIZE // 12)
...
def draw_hint(screen, hint_action):
    r, c = action_to_rc(hint_action)
    x = OFFSET + c * CELL_SIZE
    y = OFFSET + r * CELL_SIZE
    pygame.draw.circle(screen, HINT_COLOR, (x, y), STONE_R, HINT_RING_W)
    pygame.draw.circle(screen, HINT_COLOR, (x, y), max(3, STONE_R // 4))


def draw_status(screen, font, text, show_hint=False, hint_busy=False):
    screen.fill(STATUS_BG, (0, HEIGHT, WIDTH, STATUS_BAR_H))

    hint_rect = None
    if show_hint:
        mouse_pos = pygame.mouse.get_pos()
        label = "Thinking..." if hint_busy else "Hint"
        btn_w = font.size(label)[0] + 20
        btn_h = max(24, STATUS_BAR_H - 20)
        btn_x = WIDTH - 12 - btn_w
        btn_y = HEIGHT + (STATUS_BAR_H - btn_h) // 2
        hint_rect = pygame.Rect(btn_x, btn_y, btn_w, btn_h)
        col = (60, 60, 60) if hint_busy else (BUTTON_HOVER if hint_rect.collidepoint(mouse_pos) else BUTTON_COLOR)
        pygame.draw.rect(screen, col, hint_rect, border_radius=7)
        s = font.render(label, True, (255, 255, 255))
        screen.blit(s, s.get_rect(center=hint_rect.center))

    surf = font.render(text, True, STATUS_TEXT)
    txt_rect = surf.get_rect(midleft=(12, HEIGHT + STATUS_BAR_H // 2))
    if hint_rect:
        txt_rect.right = min(txt_rect.right, hint_rect.left - 8)
...
def play_match(p1_type, p2_type, p1_path=None, p2_path=None):
    ...
    # Network used for hints (whichever agent net is loaded)
    hint_net = nets[1] or nets[2]

    def run_hint(g, net):
        pi = mcts(g.clone(), net, Node(prior=1.0), root_noise=False, n_sims=400)
        hint_action[0] = int(np.argmax(pi))
        hint_thinking[0] = False
    ...
    running  = True
    back_btn = fwd_btn = restart_btn = quit_btn = hint_btn = None
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if state == 'done':
                    ...
                    # Hint button click
                    if hint_btn and hint_btn.collidepoint(event.pos) \
                            and not hint_thinking[0] and hint_action[0] is None \
                            and hint_net is not None:
                        hint_thinking[0] = True
                        threading.Thread(target=run_hint,
                                         args=(game, hint_net), daemon=True).start()
                    else:
                        mx, my = event.pos
                        if my < HEIGHT:
                            row, col = px_to_cell(mx, my)
        ...
        if state == 'done':
            ...
            # Draw hint highlight if available and it's still the human's turn
            if hint_action[0] is not None and modes[p] == 'human' \
                    and game.board[action_to_rc(hint_action[0])[0],
                                   action_to_rc(hint_action[0])[1]] == 0:
                draw_hint(screen, hint_action[0])

            if (modes[p] == 'human' or waiting_first) and my < HEIGHT:
                draw_hover(screen, game.board, mx, my)

            can_hint = (hint_net is not None and modes[p] == 'human'
                        and not waiting_first)
            if waiting_first:
                hint_btn = draw_status(screen, font_status,
                                       "Select Black's opening move")
            elif modes[p] == 'human':
                who = "Black (you)" if p == 1 else "White (you)"
                hint_btn = draw_status(screen, font_status,
                                       f"{who} \u2014 click to place",
                                       show_hint=can_hint,
                                       hint_busy=hint_thinking[0])
            else:
                # Clear hint when it becomes the agent's turn
                hint_action[0] = None
                dots = "." * (1 + (pygame.time.get_ticks() // 400) % 3)
                who  = "Black" if p == 1 else "White"
                hint_btn = draw_status(screen, font_status,
                                       f"Agent ({who}) thinking{dots}")
