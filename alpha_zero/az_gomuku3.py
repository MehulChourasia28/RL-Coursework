...
Changes from original:
  - Game: last_moves stack + undo_move() added to support batched self-play rollbacks
  - Config: GRAD_CLIP = 1.0 added
  - play_game replaced with play_games_batched (parallel GPU-batched inference)
  - train_step: clip_grad_norm_ added between backward() and step(), float32 fixes
  - Main loop: Curriculum MCTS added (100 -> 200 -> 400 sims based on epoch)
"""
...
# N_SIMS is now handled dynamically in the training loop
...
CHECKPOINT_PATH = "models3_new/checkpoint.pt"
BUFFER_PATH     = "models3_new/buffer.pkl"
...
def mcts(game: Gomoku, net: AZNet, root: Node, root_noise: bool = True, n_sims: int = 200) -> np.ndarray:
...
def train(n_iters: int = 100):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        opt, milestones=[50, 70, 85], gamma=0.5)
    ...
    for it in range(start_iter, n_iters + 1):
        ...
        # Curriculum: ramp up sims as network improves
        if it <= 10:
            current_n_sims = 100
        elif it <= 30:
            current_n_sims = 200
        ...
        net.eval()
        buf.extend(play_games_batched(net, n_games=GAMES_PER_ITER, n_sims=current_n_sims))
        ...
        print(f"Iter {it:3d} | buf={len(buf):6d} | sims={current_n_sims:3d} | loss={total/TRAIN_STEPS:.4f} | lr={current_lr:.4f}")

        torch.save(net.state_dict(), f"models3_new/az_iter{it:04d}.pt")
