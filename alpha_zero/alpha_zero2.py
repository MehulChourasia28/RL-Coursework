...
def _backup(path: list, value: float):
    for node in reversed(path):
        value = -value      # [FIXED]: Flip FIRST
        node.N += 1
        node.W += value

# updated
def mcts(game: Gomoku, net: AZNet, root: Node, root_noise: bool = True) -> np.ndarray:
    ...
    for a, p in original_priors.items():   # restore — works even if dict is empty
        root.children[a].P = p
...
def play_game(net: AZNet) -> list[tuple]:
    ...
    game    = Gomoku()
    history = []
    root    = Node(prior=1.0) # [ADDED]: Persistent root
...
def train(n_iters: int = 100):
    os.makedirs("az_models", exist_ok=True)
    ...
    for it in range(1, n_iters + 1):
        ...
        torch.save(net.state_dict(), f"az_models/az_iter{it:04d}.pt")
